...
# --- IMPORTANTE: Gerar um secret_key dinâmico para desenvolvimento ---
# Para produção, você DEVE definir um secret_key fixo e seguro,
# por exemplo, lendo de uma variável de ambiente ou arquivo de configuração.
# Exemplo para produção: app.secret_key = os.environ.get('FLASK_SECRET_KEY', 'fallback_secret_key_muito_segura')
# Para desenvolvimento, podemos gerar um novo a cada reinício para invalidar sessões antigas.
app.secret_key = os.urandom(24) # Gera uma chave aleatória de 24 bytes a cada inicialização
...
MATERIAL_REQUESTS_PER_PAGE = 10 # Define quantos requisições de material por página
...
# Configurações para upload de fotos de requisição de material
UPLOAD_FOLDER_MATERIAL_REQUEST_IMAGES = 'static/material_request_images'

ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif', 'pdf', 'doc', 'docx', 'txt'} # Adicionado tipos de arquivo para anexos
ALLOWED_IMAGE_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif'} # Extensões permitidas para imagens
...
app.config['UPLOAD_FOLDER_MATERIAL_REQUEST_IMAGES'] = UPLOAD_FOLDER_MATERIAL_REQUEST_IMAGES

def allowed_file(filename):
    """Verifica se a extensão do arquivo é permitida para anexos gerais."""
    return '.' in filename and \
           filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

def allowed_image_file(filename):
    """Verifica se a extensão do arquivo é permitida para imagens."""
    return '.' in filename and \
...
def init_db():
    """
    Inicializa o banco de dados, criando as tabelas 'users', 'calls', 'stock_items', 'notifications'
    e a nova tabela 'material_requests'.
    ...
    if not os.path.exists(app.config['UPLOAD_FOLDER_MATERIAL_REQUEST_IMAGES']):
        os.makedirs(app.config['UPLOAD_FOLDER_MATERIAL_REQUEST_IMAGES'])
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS calls (
            ...
            status TEXT NOT NULL DEFAULT 'Aberto', -- Ex: Aberto, Em Andamento, Concluído, Cancelado, Solicitação de Compras, Em Processo de Compra, Disponível / Aguardando Execução
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS notifications (
            ...
            notification_type TEXT, -- Tipo de notificação (ex: 'new_call', 'call_update', 'user_management', 'material_request')
            call_id INTEGER, -- Adiciona call_id para referência direta ao chamado
            request_id INTEGER, -- Adiciona request_id para referência direta à requisição de material
            FOREIGN KEY (user_id) REFERENCES users (id),
    ...
    # NOVA TABELA para Requisições de Material
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS material_requests (
            ...
            unit TEXT,
            link TEXT,
            image_path TEXT, -- Caminho para a imagem do material
            status TEXT NOT NULL DEFAULT 'Pendente', -- Pendente, Aprovada, Rejeitada, Em Compra, Recebida
...
def add_notification(user_id, message, link=None, notification_type=None, call_id=None, request_id=None):
    """
    Adiciona uma nova notificação ao banco de dados para um usuário específico.
    Now accepts call_id and request_id.
    """
    conn = get_db_connection()
    try:
        print(f"DEBUG: add_notification - Inserindo: user_id={user_id}, message='{message}', call_id={call_id}, request_id={request_id}")
        conn.execute(
            "INSERT INTO notifications (user_id, message, link, notification_type, call_id, request_id) VALUES (?, ?, ?, ?, ?, ?)",
            (user_id, message, link, notification_type, call_id, request_id)
        )
        conn.commit()
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password_hash'], password):
            ...
            session.clear() # Limpa qualquer sessão antiga para garantir uma nova
...
def dashboard():
    ...
    # --- Verificação de validade da sessão no DB ---
    conn = get_db_connection()
    current_user_id = session['user_id']
    user_in_db = conn.execute("SELECT id, role, is_primary_admin FROM users WHERE id = ?", (current_user_id,)).fetchone()
    conn.close()

    if not user_in_db:
        # Usuário da sessão não encontrado no DB (ex: DB foi apagado e recriado)
        session.clear()
        flash('Sua sessão expirou ou o usuário não existe mais. Por favor, faça login novamente.', 'warning')
        return redirect(url_for('login'))
    
    # Atualiza a sessão com dados frescos do DB para garantir consistência
    # Isso é importante se o papel ou status do usuário foi alterado por um admin,
    # ou se o DB foi recriado e um novo usuário com o mesmo ID foi atribuído.
    session['user_role'] = user_in_db['role']
    session['is_primary_admin'] = user_in_db['is_primary_admin']
    ...
    if user_role == 'USUARIO':
        ...
        recent_calls = conn.execute(
            """
            SELECT * FROM calls 
            WHERE user_id = ? AND status IN ('Aberto', 'Em Andamento', 'Solicitação de Compras', 'Em Processo de Compra', 'Disponível / Aguardando Execução')
        ...
        total_calls = conn.execute(
            "SELECT COUNT(*) FROM calls WHERE user_id = ? AND status IN ('Aberto', 'Em Andamento', 'Solicitação de Compras', 'Em Processo de Compra', 'Disponível / Aguardando Execução')",
            (user_id,)
        ).fetchone()[0]
        ...
        recent_calls = conn.execute(
            ...
            WHERE c.status IN ('Aberto', 'Em Andamento', 'Disponível / Aguardando Execução')
        ...
        total_calls = conn.execute(
            "SELECT COUNT(*) FROM calls WHERE status IN ('Aberto', 'Em Andamento', 'Disponível / Aguardando Execução')"
        ).fetchone()[0]
...
def register_user():
    ...
    if request.method == 'POST':
        ...
        if 'profile_image' in request.files:
            ...
            if file.filename != '' and allowed_file(file.filename): # Usar allowed_file para imagens de perfil
...
def call_details(call_id):
    ...
    # Buscar requisições de material para este chamado
    material_requests = conn.execute(
        """
        SELECT mr.*, u.name as requested_by_name, a.name as approved_by_name, r.name as received_by_name
        FROM material_requests mr
        JOIN users u ON mr.requested_by_user_id = u.id
        LEFT JOIN users a ON mr.approved_by_admin_id = a.id
        LEFT JOIN users r ON mr.received_by_admin_id = r.id
        WHERE mr.call_id = ? ORDER BY mr.requested_at DESC
        """,
        (call_id,)
    ).fetchall()


    # Buscar técnicos disponíveis para atribuição (apenas para ADMIN)
    available_technicians = []
    # Agora, apenas ADMIN pode atribuir técnico
    if session['user_role'] == 'ADMIN':
    ...
    return render_template('call_details.html', 
                           ...
                           material_requests=material_requests, # Passa requisições de material
...
@app.route('/download_material_image/<filename>')
def download_material_image(filename):
    """
    Permite o download de uma imagem de requisição de material.
    """
    return send_from_directory(app.config['UPLOAD_FOLDER_MATERIAL_REQUEST_IMAGES'], filename, as_attachment=True)
...
def attend_call(call_id):
    ...
    Apenas TECNICOS podem acessar esta rota e apenas para chamados 'Aberto', 'Em Andamento' ou 'Disponível / Aguardando Execução'.
    """
    ...
    # Verifica se o chamado está em um status que permite atendimento
    if call['status'] not in ['Aberto', 'Em Andamento', 'Disponível / Aguardando Execução']:
        flash(f'Este chamado está com status "{call["status"]}". Não pode ser atendido agora.', 'warning')
        conn.close()
...
def delete_material(item_id):
    ...
    try:
        # CORREÇÃO AQUI: String SQL terminada corretamente
...
def manage_stock():
    ...
    page = request.args.get('page', 1, type=int) 
    search_query = request.args.get('search', '').strip() # NOVO: Obtém o termo de busca
    offset = (page - 1) * ITEMS_PER_PAGE 

    conn = get_db_connection()
    
    base_query = "SELECT * FROM stock_items"
    count_query = "SELECT COUNT(*) FROM stock_items"
    query_params = []
    count_params = []
    ...
    stock_items = conn.execute(base_query, tuple(query_params)).fetchall()
    ...
    return render_template('manage_stock.html', 
                           ...
                           page=page, 
                           total_pages=total_pages,
                           search_query=search_query) # NOVO: Passa o termo de busca para o template
...
def get_notifications():
    ...
    notifications_raw = conn.execute(
        """
        SELECT n.id, n.message, n.link, n.is_read, n.created_at, n.notification_type, n.request_id,
               c.title AS call_title, c.status AS call_status, c.created_at AS call_created_at,
               c.finished_at AS call_finished_at,
               mr.description AS request_description, mr.status AS request_status
    ...
    for n in notifications_raw:
        ...
        if notification_dict['notification_type'] in ['new_call', 'call_update']:
            ...
            notification_dict['item_creation_time'] = notification_dict['call_created_at'] # Renomeado para generalizar
            ...
            if notification_dict['call_status'] == 'Concluído' and notification_dict['call_finished_at']:
                try:
                    ...
                    pass 
        elif notification_dict['notification_type'] == 'material_request':
            notification_dict['title'] = f"Requisição: {notification_dict['request_description']}"
            notification_dict['status'] = notification_dict['request_status']
            notification_dict['item_creation_time'] = notification_dict['created_at'] # Usa a data de criação da notificação para requisições
        else:
            ...
            notification_dict['item_creation_time'] = notification_dict['created_at']
...
def admin_reports():
    """
    Página de índice para os relatórios do administrador.
    Apenas ADMINs podem acessar esta rota.
    """
    if 'user_id' not in session or session['user_role'] != 'ADMIN':
...
def assign_technician(call_id):
    """
    Permite que um ADMIN atribua um chamado a um técnico.
    TECNICOS NÃO podem atribuir chamados.
    """
    if 'user_id' not in session or session['user_role'] != 'ADMIN': # Apenas ADMIN pode atribuir
        flash('Acesso negado. Apenas administradores podem atribuir chamados.', 'danger')
        return redirect(url_for('login'))
    ...
    if call['technician_id'] and call['status'] != 'Aberto': # Removido a condição de session['user_id']
...
# --- Rotas para Requisição de Material ---

@app.route('/request_material/<int:call_id>', methods=['GET', 'POST'])
def request_material(call_id):
    """
    Rota para o técnico requisitar material para um chamado.
    GET: Exibe o formulário de requisição.
    POST: Processa a requisição e muda o status do chamado.
    Apenas TECNICOS podem acessar esta rota.
    """
    if 'user_id' not in session or session['user_role'] != 'TECNICO':
        flash('Acesso negado. Apenas técnicos podem requisitar materiais.', 'danger')
        return redirect(url_for('login'))
    ...
    # Verifica se o chamado já tem uma requisição de material pendente ou em processo
    pending_request = conn.execute(
        "SELECT id FROM material_requests WHERE call_id = ? AND status IN ('Pendente', 'Em Compra')",
        (call_id,)
    ).fetchone()

    if pending_request:
        flash('Já existe uma requisição de material pendente ou em processo para este chamado.', 'warning')
        conn.close()
        return redirect(url_for('call_details', call_id=call_id))
    ...
    if request.method == 'POST':
        ...
        if not description or not quantity or not unit:
            flash('Descrição, quantidade e unidade são obrigatórios para a requisição.', 'danger')
            conn.close()
            return render_template('request_material.html', call=call)
        ...
        if 'image' in request.files:
            ...
            if file.filename != '' and allowed_image_file(file.filename):
                file_extension = file.filename.rsplit('.', 1)[1].lower()
                unique_filename = str(uuid4()) + '.' + file_extension
                file_path = os.path.join(app.config['UPLOAD_FOLDER_MATERIAL_REQUEST_IMAGES'], unique_filename)
                file.save(file_path)
        ...
        try:
            ...
            cursor.execute(
                """
                INSERT INTO material_requests (call_id, requested_by_user_id, description, brand, quantity, unit, link, image_path, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (call_id, session['user_id'], description, brand, quantity, unit, link, image_path, 'Pendente')
            )
            new_request_id = cursor.lastrowid

            # Atualiza o status do chamado para 'Solicitação de Compras'
            conn.execute(
                "UPDATE calls SET status = ? WHERE id = ?",
                ('Solicitação de Compras', call_id)
            )
            conn.commit()

            flash('Requisição de material enviada com sucesso!', 'success')

            # Notificar administradores sobre a nova requisição
            admin_users = conn.execute("SELECT id FROM users WHERE role = 'ADMIN'").fetchall()
            for admin in admin_users:
                message_admin = f"Nova requisição de material para o chamado #{call_id}: '{description}'."
                link_admin = url_for('admin_material_requests', request_id=new_request_id) # Link para a página de gerenciamento de requisições
                add_notification(admin['id'], message_admin, link_admin, 'material_request', call_id, new_request_id)

            return redirect(url_for('call_details', call_id=call_id))
        except Exception as e:
            flash(f'Erro ao enviar requisição de material: {e}', 'danger')
            print(f"Erro ao enviar requisição de material: {e}")
            conn.rollback()
        finally:
            conn.close()
...
@app.route('/admin_material_requests')
def admin_material_requests():
    """
    Rota para Administradores visualizarem e gerenciarem requisições de material.
    Apenas ADMINs podem acessar esta rota.
    """
    if 'user_id' not in session or session['user_role'] != 'ADMIN':
        flash('Acesso negado. Apenas administradores podem gerenciar requisições de material.', 'danger')
        return redirect(url_for('login'))

    page = request.args.get('page', 1, type=int)
    status_filter = request.args.get('status_filter', '').strip()
    search_query = request.args.get('search', '').strip()
    offset = (page - 1) * MATERIAL_REQUESTS_PER_PAGE

    conn = get_db_connection()

    base_query = """
        SELECT mr.*, c.title as call_title, u.name as requested_by_name
        FROM material_requests mr
        JOIN calls c ON mr.call_id = c.id
        JOIN users u ON mr.requested_by_user_id = u.id
    """
    count_query = "SELECT COUNT(*) FROM material_requests mr JOIN calls c ON mr.call_id = c.id JOIN users u ON mr.requested_by_user_id = u.id"
    query_params = []
    count_params = []
    ...
    if search_query:
        conditions.append("(mr.description LIKE ? OR c.title LIKE ? OR u.name LIKE ?)")
    ...
    base_query += " ORDER BY mr.requested_at DESC LIMIT ? OFFSET ?"
    query_params.extend([MATERIAL_REQUESTS_PER_PAGE, offset])

    material_requests = conn.execute(base_query, tuple(query_params)).fetchall()
    total_requests = conn.execute(count_query, tuple(count_params)).fetchone()[0]
    
    conn.close()

    total_pages = (total_requests + MATERIAL_REQUESTS_PER_PAGE - 1) // MATERIAL_REQUESTS_PER_PAGE

    return render_template('admin_material_requests.html',
...
@app.route('/approve_material_request/<int:request_id>', methods=['POST'])
def approve_material_request(request_id):
    """
    Rota para o Administrador aprovar uma requisição de material.
    Muda o status da requisição para 'Em Compra' e o status do chamado para 'Em Processo de Compra'.
    """
    if 'user_id' not in session or session['user_role'] != 'ADMIN':
        flash('Acesso negado. Apenas administradores podem aprovar requisições.', 'danger')
        return redirect(url_for('login'))

    conn = get_db_connection()
    request_item = conn.execute("SELECT * FROM material_requests WHERE id = ?", (request_id,)).fetchone()

    if not request_item:
        flash('Requisição de material não encontrada.', 'danger')
        conn.close()
        return redirect(url_for('admin_material_requests'))

    if request_item['status'] != 'Pendente':
        flash(f'Esta requisição já está "{request_item["status"]}". Não pode ser aprovada.', 'warning')
        conn.close()
        return redirect(url_for('admin_material_requests'))
    ...
    try:
        ...
        conn.execute(
            ...
            WHERE id = ?
            """,
            ('Em Compra', session['user_id'], datetime.now().strftime('%Y-%m-%d %H:%M:%S'), request_id)
        )
        # Atualiza o status do chamado associado
        conn.execute(
            "UPDATE calls SET status = ? WHERE id = ?",
            ('Em Processo de Compra', request_item['call_id'])
        )
        conn.commit()
        flash(f'Requisição de material #{request_id} aprovada e em processo de compra.', 'success')

        # Notificar o técnico que fez a requisição
        message_tech = f"Sua requisição de material '{request_item['description']}' para o chamado #{request_item['call_id']} foi APROVADA e está 'Em Processo de Compra'."
        link_tech = url_for('call_details', call_id=request_item['call_id'])
        add_notification(request_item['requested_by_user_id'], message_tech, link_tech, 'material_request', request_item['call_id'], request_id)

        return redirect(url_for('admin_material_requests'))
    except Exception as e:
        flash(f'Erro ao aprovar requisição: {e}', 'danger')
        print(f"Erro ao aprovar requisição: {e}")
        conn.rollback()
    finally:
        conn.close()

@app.route('/mark_material_received/<int:request_id>', methods=['POST'])
def mark_material_received(request_id):
    """
    Rota para o Administrador marcar uma requisição de material como recebida.
    Muda o status da requisição para 'Recebida' e o status do chamado para 'Disponível / Aguardando Execução'.
    """
    if 'user_id' not in session or session['user_role'] != 'ADMIN':
        flash('Acesso negado. Apenas administradores podem marcar materiais como recebidos.', 'danger')
        return redirect(url_for('login'))
    ...
    if request_item['status'] != 'Em Compra':
        flash(f'Esta requisição está com status "{request_item["status"]}". Não pode ser marcada como recebida.', 'warning')
        conn.close()
        return redirect(url_for('admin_material_requests'))
    ...
    try:
        ...
        conn.execute(
            """
            UPDATE material_requests
            SET status = ?, received_by_admin_id = ?, received_at = ?
            WHERE id = ?
            """,
            ('Recebida', session['user_id'], datetime.now().strftime('%Y-%m-%d %H:%M:%S'), request_id)
        )
        # Atualiza o status do chamado associado
        conn.execute(
            "UPDATE calls SET status = ? WHERE id = ?",
            ('Disponível / Aguardando Execução', request_item['call_id'])
        )
        conn.commit()
        flash(f'Material para requisição #{request_id} recebido. Chamado agora "Disponível / Aguardando Execução".', 'success')

        # Notificar o técnico que fez a requisição
        message_tech = f"O material para sua requisição '{request_item['description']}' do chamado #{request_item['call_id']} foi RECEBIDO! O chamado está agora 'Disponível / Aguardando Execução'."
        link_tech = url_for('call_details', call_id=request_item['call_id'])
        add_notification(request_item['requested_by_user_id'], message_tech, link_tech, 'material_request', request_item['call_id'], request_id)

        # Notificar o usuário que abriu o chamado (opcional, mas bom para visibilidade)
        call_opener_id = conn.execute("SELECT user_id FROM calls WHERE id = ?", (request_item['call_id'],)).fetchone()[0]
        message_user = f"O material para o seu chamado '{conn.execute('SELECT title FROM calls WHERE id = ?', (request_item['call_id'],)).fetchone()[0]}' foi recebido. O técnico pode prosseguir com o serviço."
        add_notification(call_opener_id, message_user, link_user, 'call_update', request_item['call_id'])


        return redirect(url_for('admin_material_requests'))
    except Exception as e:
        flash(f'Erro ao marcar material como recebido: {e}', 'danger')
        print(f"Erro ao marcar material como recebido: {e}")
        conn.rollback()
    finally:
        conn.close()

@app.route('/reject_material_request/<int:request_id>', methods=['POST'])
def reject_material_request(request_id):
    """
    Rota para o Administrador rejeitar uma requisição de material.
    Muda o status da requisição para 'Rejeitada'.
    """
    if 'user_id' not in session or session['user_role'] != 'ADMIN':
        flash('Acesso negado. Apenas administradores podem rejeitar requisições.', 'danger')
        return redirect(url_for('login'))

    rejection_reason = request.form.get('rejection_reason', 'Motivo não especificado.')
    ...
    if request_item['status'] != 'Pendente':
        flash(f'Esta requisição já está "{request_item["status"]}". Não pode ser rejeitada.', 'warning')
        conn.close()
        return redirect(url_for('admin_material_requests'))
    ...
    try:
        conn.execute(
            ...
            WHERE id = ?
            """,
            ('Rejeitada', rejection_reason, request_id)
        )
        conn.commit()
        flash(f'Requisição de material #{request_id} rejeitada.', 'success')

        # Notificar o técnico que fez a requisição
        message_tech = f"Sua requisição de material '{request_item['description']}' para o chamado #{request_item['call_id']} foi REJEITADA. Motivo: {rejection_reason}"
        link_tech = url_for('call_details', call_id=request_item['call_id'])
        add_notification(request_item['requested_by_user_id'], message_tech, link_tech, 'material_request', request_item['call_id'], request_id)
        ...
        flash(f'Erro ao rejeitar requisição: {e}', 'danger')
        print(f"Erro ao rejeitar requisição: {e}")
        conn.rollback()
    finally:
        conn.close()
...
def admin_dashboard_summary():
    ...
    try:
        ...
        # Contagem de requisições de material pendentes
        pending_material_requests = conn.execute(
            "SELECT COUNT(*) FROM material_requests WHERE status = 'Pendente'"
        ).fetchone()[0]
        ...
        return jsonify({
            ...
            'pending_material_requests': pending_material_requests # Adicionado ao resumo
        })
    except Exception as e:
